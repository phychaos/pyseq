...
class CRF(object):
    ...
    def fit(self, x_train, y_train, model_file=None, mp=1, template_file=None, max_iter=20, n_jobs=None):
        ...
        if mp == 1:
            ...
            likelihood = lambda x: -self.likelihood_mp_sa(x, seq_lens, n_jobs)
        else:
            likelihood = lambda x: -self.likelihood_sa(x, seq_lens)
    ...
    def predict(self, x_test, y_test=None, model_file=None, res_file=None):
        ...
        if y_test:
            self.check_tagging(max_ys, y_test)
        if res_file:
    ...
    def likelihood_mp_sa(self, theta, seq_lens, n_jobs):
        """
    ...
    def likelihood_sa(self, theta, seq_lens):
        """
        损失函数 梯度
    ...
    def likelihood_thread_sa(self, theta, seq_lens, uon, bon):
        """
        计算序列特征概率
        :param seq_lens: 序列长度
